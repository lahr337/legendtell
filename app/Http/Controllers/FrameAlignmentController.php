...
class FrameAlignmentController extends Controller
{
    ...
    public function saveFrameAlignment(Request $request)
    {
        ...
        // $serviceData = FrameAlignment::where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
        // if ($serviceData) {
        //     $frame_alignment = $frame_alignment->where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
        // }
        ...
        if ($frame_alignment->save()) {
            ...
            // $serviceData = AfterFrameAlignment::where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
            // if ($serviceData) {
            //     $after_frame_alignment = $after_frame_alignment->where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
            // }
