...
class PartsController extends Controller
{
    ...
    public function indexPart(Request $request, $vinid = null, $tab = 'editProfile')
    {
        ...
        $serviceData = Parts::where('car_id', $car_id)->where('service_id', $collisionserviceId[1])->where('user_id', auth()->user()->id)->first();

        $shopsetting = new ShopSettingController;
        $VInGet = $shopsetting->FetchgetVinList($tab);
        return view('shop-settings.partials.shop_services.parts', compact('vinid', 'VInGet', 'serviceData', 'service_id', 'page_title'));
    }
    ...
    public function saveParts(Request $request)
    {
        ...
        $part =  Parts::where('car_id', $car_id)->where('user_id', auth()->user()->id)->first();


        if ($part) {
            if (!empty($part->document)) {

                $documents = explode(',', $part->document);
        ...
        if ($request->hasfile('products_uploaded')) {
            $products_uploaded = $commonClass->uplodeimages($_POST['remove_products_ids'], $request->file('products_uploaded'), 'part', $img_arr);
        } else {
            $products_uploaded = implode(" , ", array_column($img_arr, 'path'));
        }

        $parts = new Parts();
        $serviceData = Parts::where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
        if ($serviceData) {
            $parts = $parts->where('car_id', $car_id)->where('service_id', $serviceId)->where('user_id', auth()->user()->id)->first();
        }
        $parts->user_id = auth()->user()->id;
        $parts->car_id = $car_id;
        $parts->service_id = $serviceId;
        $parts->document = $products_uploaded;
        $parts->system = $request->system;
        $parts->part = $request->part;
        $parts->diagnosis = $request->diagnosis;
        $parts->is_waranty = $request->is_waranty;
        $parts->waranty_company = $request->waranty_company;
        $parts->part_number = $request->part_number;
        $parts->part_note = $request->part_note;
        $parts->manufacturer_by = $request->manufacturer_by;
