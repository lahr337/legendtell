...
class LubricationController extends Controller
{
    public function indexLubrication(Request $request, $vinid = null, $tab = 'editProfile')
    {
        ...
        $serviceData = Lubrication::where('car_id', $car_id)->where('service_id', $collisionserviceId[1])->where('user_id', auth()->user()->id)->first();

        $shopsetting = new ShopSettingController;
        $VInGet = $shopsetting->FetchgetVinList($tab);
        return view('shop-settings.partials.shop_services.lubrication', compact('vinid', 'VInGet', 'serviceData', 'service_id', 'page_title'));
    }

    public function saveLubrication(Request $request)
    {
        ...
        $lubrication = new Lubrication();
        
        $lubrication->user_id = auth()->user()->id;
        $lubrication->car_id = $car_id;
        $lubrication->service_id = $serviceId;
        $lubrication->document = $products_uploaded;
