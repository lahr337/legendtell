...
class NitrousController extends Controller
{
    public function indexNitrous(Request $request, $vinid = null, $tab = 'editProfile')
    {
        ...
        $serviceData = Nitrous::where('car_id', $car_id)->where('service_id', $collisionserviceId[1])->where('user_id', auth()->user()->id)->first();

        $shopsetting = new ShopSettingController;
        $VInGet = $shopsetting->FetchgetVinList($tab);
        return view('shop-settings.partials.shop_services.nitrous', compact('vinid', 'VInGet', 'serviceData', 'service_id', 'page_title'));
    }

    public function saveNitrous(Request $request)
    {
        ...
        $nitrous->user_id = auth()->user()->id;
        $nitrous->car_id = $car_id;
        $nitrous->service_id = $serviceId;
        $nitrous->document = $products_uploaded;
